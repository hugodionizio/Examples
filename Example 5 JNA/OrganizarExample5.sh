...
NUM=`wc -l src/Example$EXAMPLE.java`
NUMTMP="${NUM#* }"
LIM=`expr ${#NUM} - ${#NUMTMP} - 1`
NUMTOTAL=${NUM: 0: $LIM}
NUMBODY=`expr $NUMTOTAL - 1`
tail -n $NUMBODY src/Example$EXAMPLE.java >> tmp.java
rm src/Example$EXAMPLE.java
...
# Renomeação da classe Java do exemplo com a numeração correspondente
mv src/Example.java src/Example$EXAMPLE.java
